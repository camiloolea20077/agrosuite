...
export class CattleSaleFormComponent {
    ...
    loadForm(): void {
        this.frm = this.fb.group({
            ...
            pesoTotal: [{ value: null, disabled: true }, [Validators.required, Validators.min(1)]],
    ...
    async submitForm(): Promise<void> {
        if (this.frm.invalid || this.selectedItems.length === 0) {
            ...
            this._alertService.showError('Alerta del sistema', 'Complete todos los campos y seleccione al menos un animal');
            return;
        }
